...
def loadRawData(d, fn):
    ...
    for run in data['Runs']:
        runtimes = run['RuntimePerBatch']
        
        for i in range(len(runtimes)):
            d['BatchRuntime'].append(runtimes[i])
            d['Batch'].append(i)
...
def plot_ts(df, ax, color, label):
    ...
    for b in range(min_batch, max_batch, w):
        print(b, b + w, df['Batch'].max(), df['Batch'].min())
...
d = {'Batch': [], 'Run': [], 'BatchRuntime' : []}
loadRawData(d, '../ips-seq-bms-lg-0.01.json')

df = pd.DataFrame(data=d)   
df = df.sort(['Batch'])
plot_ts(df, ax1, flatui[0], 'BatchRuntime')
