...
def plot_ts(df, ax, color, yLabel, xLabel, width, label, linestyle = '-', showMax = False):
    valid_y1 = []
    valid_y2 = []
    valid_x = []
    line_x = []
...
def main(id, file_id, title, show):
    ...
    ProSecCo = '../u_ips-' + id + '.json'
    
    # eval runtime
    runtimes_ps = getRuntime(prefixspan)
    runtimes_ips = getRuntime(ProSecCo)
    ...
    d = loadNormalizedErrorData(ProSecCo)
    df = pd.DataFrame(data=d)   
    ...
    d = loadPrecisionRecallData(ProSecCo)
    df = pd.DataFrame(data=d)   
    df = df.sort_values(by=['Batch'])
    ...
    d = loadRawData(ProSecCo, 'RuntimePerBatch', 'RuntimePerBatch')
    df = pd.DataFrame(data=d)  
    df = df.sort_values(by=['Batch'])
    plot_ts(df, ax4, flatui[0], 'RuntimePerBatch', 'Batch', 1, '')
    ...
    d = loadAbsoluteErrorData(ProSecCo)
    df = pd.DataFrame(data=d)   
    df = df.sort_values(by=['Batch'])
